...
For example, we add the lifetime of each component in the above serial system in the following 


```{r echo = FALSE, fig.align='center', out.width = '50%'}
include_graphics("topic12/seriesSystem-example.png")
```
...
$$
\text{system lifetime} = \min \{7, 3, 5 \} = 3.
$$.


Next, we use an example to show how to characterize the distribution of the statistic $X_{(1)} = \text{min}\{X_1, X_2, \cdots, X_n \}$ where the independent component $\{X_1, X_2, \cdots, X_n \} \rightarrow f_i(x)$.
...
$$
f_i(x) = \lambda_i e^{-\lambda_i x}, \ \ \ \text{ for } \ \ x > 0.
$$
...
$$
F_{Y_{(1)}} (y) = 1 - e^{-(0.2 + 0.3 + 0.5)y} = 1 - e^{-y}.
$$
...
For ease of illustration, we assign a lifetime to each individual component in the following.

```{r echo = FALSE, fig.align='center', out.width = '38%'}
include_graphics("topic12/parallelSystem-example.png")
```

The lifetime of the system is 

$$
\text{system lifetime} = \max \{7, 3, 5 \} = 7.
$$

The following example demonstrates how to find the distribution of the system lifetime based on a given liftime distributions of the individual components in the parallel system.
...
We can similarly assign lifetime to individual component in the above system and get


```{r echo = FALSE, fig.align='center', out.width = '55%'}
include_graphics("topic12/combinedSystem-example.png")
```


The lifetime of the hybrid system is given by

$$
\text{system lifetime} = \max \{\min\{3, 5 \},7 \}= 7.
$$
...
$$
=\bigg( 1 - [1-(1-e^{-\lambda_1 y})]\times [1-(1-e^{-\lambda_2 y})] \bigg) \times \bigg( 1-\lambda_2 e^{-\lambda_3 y}\bigg)
$$


$$
=\bigg(1 - e^{-(\lambda_1 + \lambda_2)y} \bigg)\bigg( 1- e^{-\lambda_3 y}\bigg).
$$
...
$$
P[Y \le 1] = \bigg(1 - e^{-(0.2 + 0.3)} \bigg)\bigg( 1- e^{-0.5}\bigg) \approx 0.155.
$$

The probability that the combined system fails in one unit of time is about 15.5\%, as expected, that is between 1.85\% (for the parallel system) and 63.2\% (for the series system).
