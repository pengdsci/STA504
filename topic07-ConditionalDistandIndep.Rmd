...
* For a given the joint CDF of ($X, Y$), say, $(F(x,y))$, if we can decompose the CDF as $F(x,y) = F(x)\times F(y)$, where $F(x)$ and $F(y)$ are the corresponding CDFs of $X$ and $Y$, the $X$ and $Y$ are independent. Otherwise, they are dependent.
